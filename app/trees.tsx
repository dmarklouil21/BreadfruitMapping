...
export default function TreeListScreen() {
  ...
  const filteredTrees = useMemo(() => {
    return trees.filter(tree => {
      const matchesLocation = selectedLocation === 'All' || tree.location === selectedLocation;
      const matchesSearch = tree.location.toLowerCase().includes(searchQuery.toLowerCase());
      return matchesLocation && matchesSearch;
    });
  }, [trees, searchQuery, selectedLocation]); 

  const locations = useMemo(() => {
    const allLocations = trees.map(tree => tree.location);
